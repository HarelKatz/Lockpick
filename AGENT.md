...
- **Indicator-confidence misclassification** — `arp`, `ip_neigh`, `iptables`, and `nftables` parsers emit `ConnectionRecord`s that should render at *indicator* confidence on the graph, but `services/graph_builder.py::_classify_connection_evidence` currently classifies them as *observed* (it only recognizes `bash_history`/`known_hosts` substrings in `source_file` for indicator status). Until fixed, edges from these parsers look stronger than they are. Likely fix: add a `parser_file_type` field on `ConnectionRecord` or pass a hint argument to the classifier.
