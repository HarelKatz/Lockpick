...
3. **iptables parser silently drops `iptables-save -c` counter-prefixed rule lines.** `backend/parsers/iptables.py:77` dispatches each line by `line.startswith(("-A ", "-I ", "-N ", ":"))` to choose between the iptables-save and `iptables -L` codepaths. Rules emitted by `iptables-save -c` (or any save invocation that preserves byte/packet counters) are prefixed with `[pkts:bytes] ` — e.g. `[1:2] -A PREROUTING -j PREROUTING_direct`. After `line.strip()` they still start with `[`, so the startswith check fails and the line falls through to the `iptables -L` columnar parse below (line 100+), which then splits on whitespace and reads `parts[3]`/`parts[4]` as `source`/`destination` — those slots are actually `-j` and the chain-jump target, neither of which extracts as an IP, so the line silently drops with no warning. Net effect: any host that ran `iptables-save -c` produces `stats.rules: 0` regardless of how many rules the file contains, AND we lose the counters themselves (counters are forensic evidence — non-zero values on attacker-related rules are a signal of activity). Reproducer: `real_examples/iptables/aosp_fedora27_iptables_save` — 30+ counter-prefixed `-A` rules, parser emits zero connections and zero rule count. The other AOSP iptables-save fixtures (`aosp_wireless_pptp_helper_save`, `aosp_iptables_nft_save_complex`) are not counter-prefixed and parse correctly, which is why this gap was misattributed in the round-1 commit message as "parser doesn't support iptables-save format" — it does, just not the `-c` counter variant. Real fix: strip an optional `^\[\d+:\d+\]\s+` prefix before the startswith dispatch (and ideally extract the counter into the `raw_line` or a future `evidence_counters` field for forensic visibility), then re-run the existing iptables-save extraction on the stripped tail.
