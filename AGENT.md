...
> For the tech stack, see **README.md**. For dev commands, repo layout, migration rules, frontend conventions, and git format — see **CLAUDE.md**.
...
**Last completed: Phases 1–18 — full stack, 49 file types / 44 parser classes, host merge, one-click collection workflow**
...
Full stack: CRUD APIs, force-graph visualization, BFS pivot analysis, global search, export/import, activity log, WS live push, host merge (manual + auto-merge of placeholder hosts), one-click collection script + bulk archive import. 49 file types / 44 parser classes covering SSH artifacts, system files, command output, and credential files. Invariants worth preserving live in **Architecture Rules**; per-parser detail lives in git.
