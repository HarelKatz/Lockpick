...
1. **GraphCanvas d3-force "node not found" on node-set change.** When the node set grows or shrinks, ForceGraph2D's d3-force `forceLink.initialize()` can throw `Error: node not found: <host_id>` against a host that IS present in the new data — an internal d3-force timing issue between simulation re-init and link source/target ref resolution. `frontend/src/components/GraphCanvas.tsx` keys `<ForceGraph2D>` on `nodeSetKey` (sorted join of `host_id`s) so any node-set change unmounts/remounts with a fresh simulation, and defensively clears the link force (`d3Force('link')?.links([])`) before each `setFgData` — but neither workaround is fully reliable. The throw bubbles up to `<ErrorBoundary>` (`frontend/src/components/ErrorBoundary.tsx`), which renders "Render error in graph" with a Retry button; clicking Retry remounts cleanly with the latest data and the graph repopulates correctly. The WS-driven burst trigger is mitigated by a 250 ms trailing debounce in `frontend/src/pages/Workspace.tsx` (`scheduleReload`) — bursts of WS events coalesce into one reload, so a 12-event bulk import goes through one structural update instead of twelve racing ones — but the d3 race can still surface on individual node-set changes (single host add, single delete). Real fix: replace the keyed-remount workaround with an incremental d3-force update (or a forced `graphRef.current?.refresh()` after each `setFgData`) once the d3-force "node not found" crash is solved upstream — likely a `react-force-graph-2d` issue rather than something fixable purely from app code.
