...
- **Test honesty** — a live-push e2e test asserted `getByText(nickname).first()` and never passed, which got filed as an app bug ("live push connects but the Data tab never refreshes"). There was no bug. `Workspace` keeps **both** panels mounted and hides the inactive one with `visibility: hidden` (deliberate — ForceGraph's ResizeObserver must keep measuring), so a host renders twice: once in the graph panel's filter list, once in the Data panel. The graph copy is first in DOM order, so `.first()` deterministically picked a permanently-invisible element. Instrumenting the boundaries (WS frames → refetch → API body → DOM) showed the frame arriving, the refetch firing, and the nickname present in the DOM the whole time. Fixed by scoping to `getByRole('main')` → `frontend/e2e/ws-live-push.spec.ts`. Lesson: with both panels always mounted, an **unscoped text locator is ambiguous by construction** — scope to the active panel, and treat "element exists but is never visible" as a locator bug before believing it is an app bug.
- **Layout / rendering** — the same host OS/kernel wrap test then passed alone and **failed in the full suite**: it measured the chip and its section in two separate `boundingBox()` calls, which straddle the detail sidebar's open transition — the panel drifts ~95px horizontally between samples, so the two boxes come from different frames and the comparison is arithmetic on unrelated numbers. Green or red was pure timing. Fixed by settling the sidebar (poll until its left edge stops moving) and taking every measurement in **one** `page.evaluate` → `frontend/e2e/host-system-info.spec.ts`. Lesson: geometry from two round-trips is two frames; anything that animates must be settled first and sampled atomically.
