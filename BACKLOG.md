...
- **Retroactive changes to an op — audit trail + durability** — standing rules (Rule #28) mutate an op's graph *after* the operator's action: uploading a bash_history can materialize edges from an ACL ingested days earlier, against a host neither of them mentioned. Two gaps worth a look. **(a) No audit trail** — those edges are folded into the upload's single `new_connections` count (`routers/upload.py`), and `apply_patterns_to_host` logs nothing of its own, so an operator cannot tell which edges came from the file they just uploaded and which from a rule firing retroactively. Wants a distinct activity entry naming the rule + matched host (cf. the per-merge `host.auto_merge` precedent, Rule #23) and probably an indication on the edge itself. **(b) Rules don't survive export/import** — `SshConfigPattern` is absent from the export schema, a documented drop (`tests/test_api/test_export_import.py:270`), so an imported op keeps the already-materialized edges but loses every rule and silently stops matching hosts discovered later. Phase 4 widened this: `authorized_keys` `from=` ACLs live in that same table and are dropped too. Note the ordering trap — `routers/export_import.py:139` creates hosts without applying rules, which is harmless *only* because no rules are imported; fixing the export without also applying rules on import would leave imported hosts unmatched.
